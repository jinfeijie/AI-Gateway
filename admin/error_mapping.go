...
func registerErrorMappingRoutes(r *gin.RouterGroup, s *store.Store) {
	...
	// 分组级字段注入
	r.GET("/groups/:id/inject-fields", func(c *gin.Context) {
		id := c.Param("id")
		cfg := s.Get()
		for _, g := range cfg.Groups {
			if g.ID == id {
				c.JSON(http.StatusOK, gin.H{"data": g.InjectFields})
				return
			}
		}
		c.JSON(http.StatusNotFound, gin.H{"error": "group not found"})
	...
	r.PUT("/groups/:id/inject-fields", func(c *gin.Context) {
		id := c.Param("id")
		var req []model.InjectField
		if err := c.ShouldBindJSON(&req); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
	...
	r.GET("/inject-fields", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"data": s.Get().InjectFields})
	})

	r.PUT("/inject-fields", func(c *gin.Context) {
		var req []model.InjectField
		if err := c.ShouldBindJSON(&req); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
